...
This tutorial uses several spatial packages that require system libraries. If those dependencies are not installed locally, code chunks are skipped so site rendering can still complete.
...
```{r depot-map, warning=FALSE, message=FALSE, eval=spatial_ready}
...
```{r dock-map, warning=FALSE, message=FALSE, eval=spatial_ready}
...
```{r survey-raster, warning=FALSE, message=FALSE, eval=spatial_ready}
