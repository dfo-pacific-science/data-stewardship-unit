...
```{r setup-librarian, message=FALSE, warning=FALSE, error=FALSE}
spatial_packages <- c(
...
```{r dependency-note, results='asis', echo=FALSE}
if (!spatial_ready) {
  cat("> Note: Spatial code examples are skipped in this render because required system dependencies are not installed locally.\n")
}
if (!ckan_ready) {
  cat("> Note: CKAN API examples are skipped in this render because `ckanr` is not installed.\n")
}
```

```{r map-cache, include=FALSE}
map_cache_dir <- "../images/openDataSpatial/rendered"
dir.create(map_cache_dir, recursive = TRUE, showWarnings = FALSE)

use_cached_map <- function(filename, plot_expr = NULL, width = 10, height = 7, dpi = 150, force = FALSE) {
  out_file <- file.path(map_cache_dir, filename)

  if ((force || !file.exists(out_file)) && !is.null(plot_expr)) {
    if (!spatial_ready) {
      stop("Cannot generate map cache because required spatial packages are unavailable.")
    }
    p <- force(plot_expr)
    ggplot2::ggsave(out_file, plot = p, width = width, height = height, dpi = dpi)
  }

  if (!file.exists(out_file)) {
    stop("Cached map not found: ", out_file)
  }
...
```{r salmonDepots, warning=FALSE, error=FALSE, eval=spatial_ready}
...
```{r coastline, warning=FALSE, message=FALSE, eval=spatial_ready}
...
```{r depot-map, warning=FALSE, message=FALSE, echo=FALSE}
if (spatial_ready) {
  ...
  use_cached_map("depot-map.png", plot_expr = depot_plot)
} else {
  use_cached_map("depot-map.png")
}
```
...
```{r tempDirs, warning=FALSE, message=FALSE, eval=spatial_ready}
...
```{r dock-map, warning=FALSE, message=FALSE, echo=FALSE}
if (spatial_ready) {
  ...
  use_cached_map("dock-map.png", plot_expr = dock_plot)
} else {
  use_cached_map("dock-map.png")
}
```
...
```{r temp-vessels, echo = TRUE, message= FALSE, warning= FALSE, error= FALSE, cache= FALSE, results='hide', eval=spatial_ready}
...
```{r survey-raster, warning=FALSE, message=FALSE, echo=FALSE}
if (spatial_ready) {
  ...
  use_cached_map("survey-raster.png", plot_expr = raster_plot)
} else {
  use_cached_map("survey-raster.png")
}
```
...
```{r leaflet, warning=FALSE, message=FALSE, eval=spatial_ready}
...
```{r ckanr, warning=FALSE, message=FALSE, eval=ckan_ready}
...
```{r searching, echo = TRUE, message= FALSE, warning= FALSE, error= FALSE, cache= FALSE, results='hide', eval=ckan_ready}
