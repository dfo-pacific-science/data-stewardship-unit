...
# Function to load themes from ontology config
load_themes_from_config <- function(project_root) {
  themes_yml_path <- file.path(project_root, "data/ontology/scripts/config/themes.yml")
  
  if (!file.exists(themes_yml_path)) {
    warning("themes.yml not found at ", themes_yml_path, ", using fallback themes")
    # Fallback to hardcoded themes if config file not found
    return(list(
      ...
      list(id = "benchmarks-terms", label = "Biological Benchmarks and Indicators"),
  ...
  themes_config <- tryCatch({
    ...
    warning("Failed to read themes.yml: ", e$message, ", using fallback themes")
    return(NULL)
  })
  
  if (is.null(themes_config) || is.null(themes_config$themes)) {
    warning("themes.yml is empty or invalid, using fallback themes")
    return(list(
      list(id = "stock-assessment-terms", label = "Stock Assessment Methods"),
      list(id = "genetics-terms", label = "Genetics and GSI"),
      list(id = "legislation-terms", label = "Legislation and Policy"),
  ...
  # Convert YAML structure to R list format
  # Use output_csv field directly (already has correct hyphenated format)
  themes <- lapply(themes_config$themes, function(theme) {
    # Extract theme ID from CSV filename (remove .csv extension)
...
themes <- load_themes_from_config(project_root)
...
# Function to load themes from ontology config (with optional desc field)
load_themes_from_config_with_desc <- function(project_root) {
  themes_yml_path <- file.path(project_root, "data/ontology/scripts/config/themes.yml")
  ...
  # Convert YAML structure to R list format with optional desc field
  themes <- lapply(themes_config$themes, function(theme) {
    # Extract theme ID from CSV filename (remove .csv extension)
    csv_filename <- theme$output_csv %||% paste0(theme$id, ".csv")
    theme_id <- sub("\\.csv$", "", csv_filename)
    ...
    list(
      id = theme_id,
      label = theme$label,
      desc = theme$desc %||% ""  # Optional description field
    )
  })
...
# Load themes dynamically
themes <- load_themes_from_config_with_desc(project_root)
...
for (idx in seq_along(themes)) {
  ...
  if (is.null(df) || nrow(df) == 0) {
    ...
    # Add theme column to dataframe - ensure correct theme label is assigned
    theme_label <- theme$label
    df$theme <- theme_label
    
    # Render table using reactable - this will be detected by Quarto
    table_output <- render_table(df, theme_label = theme_label)
