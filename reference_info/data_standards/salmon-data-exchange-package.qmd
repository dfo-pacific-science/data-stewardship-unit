...
```bash
Rscript scripts/validate_spsr_package.R path/to/salmon-data-package
```

That helper calls the new end-to-end validator:
...
Use `--require-iris` (or `require_iris = TRUE`) when you want the pre-flight check to fail on missing measurement IRIs.
